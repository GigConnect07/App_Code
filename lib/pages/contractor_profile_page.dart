...
class _ContractorProfilePageState extends State<ContractorProfilePage> {
  ...
  void initState() {
    ...
    if (_user != null) {
      _userRef = _firestore.collection('Contractor').doc(_user!.uid);
      _loadUserData();
      _loadProfilePic();
    }
  }
  ...
  Future<void> _loadProfilePic() async {
    ...
    if (uid == null) return;
    final doc = await _firestore.collection('Contractor').doc(uid).get();
    setState(() {
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              ...
              if (!_isEditing) {
                _loadUserData(); // Reload data if cancelling edit
              }
            },
          ),
        ],
